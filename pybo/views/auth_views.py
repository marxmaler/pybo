...
def login():
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        if error is None:
            session.clear #flask.session import
...
@bp.before_app_request #route 함수보다 먼저 실행됨
def load_logged_in_user():
    user_id = session.get('user_id')
    if user_id is None:
        g.user = None #g는 플라스크 패키지의 변수
    else:
        g.user = User.query.get(user_id) # 세션 정보에 포함된 아이디로 데이터베이스에서 아이디에 딸린 정보들 불러옴
